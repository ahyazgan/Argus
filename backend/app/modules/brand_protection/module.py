"""Marka koruma modulu - ArgusModule uygulamasi ve kayit."""
...
from app.modules.brand_protection.analyzer import analyze as analyze_brand
from app.modules.brand_protection.collectors import get_collectors
from app.modules.catalog import CATALOG_BY_KEY


class BrandProtectionModule(ArgusModule):
    meta: ModuleMeta = CATALOG_BY_KEY["brand_protection"]
    ...
    async def scan(self, monitor) -> list[ScanResult]:
        ...
        for collector in get_collectors():
            for raw in collector.collect(monitor.asset_type, monitor.asset_value):
                results.append(
                    ScanResult(
                        title=f"{raw.get('variant_domain', monitor.asset_value)} - taklit alan adi",
                        raw_data=raw,
    ...
    def analyze(self, result: ScanResult, monitor) -> TriageResult:
        return analyze_brand(result, monitor.asset_type)


# Modulu global kayit defterine ekle (load_modules() bu dosyayi import eder)
register(BrandProtectionModule())
